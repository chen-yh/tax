      ...
      <div slot="datas" v-for="(r,index) in dRepayDatas" :key="index" v-show="mainData">
        ...
        <dd slot="countData">
          <div class="countData">
            ...
            <div><router-link to="outRepaymentInfo" class="blue">查看详情 >> </router-link></div>
          </div>
        </dd>
      </div>
    </tables-tit-more>
    ...
    <tables-tit-more :tablesData="repayList">
      ...
      <div slot="datas" v-for="(r,index) in repayDatas" :key="index" v-show="dmainData" id="tables">
        <dd>
          ...
          <div>{{r.refundApply.loanTime}}</div>
          <div>{{r.refundApply.expireTime}}</div>
          <div>
          ...
          <div>{{r.refundApply.deadDays}}</div>
    ...
    <!--统计-->
      ...
      <div class="bg-hui clear">
        ...
        <div class="go-repayment">
          合计：<span id="repayMoney">￥590000</span>
...
export default({
  ...
  methods: {
    ...
    oneCheckbox () {
      ...
      if ($('input:checkbox:checked').not('#all').length === $('input:checkbox').not('#all').length) {
