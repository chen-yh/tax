  ...
  <div class="detail">
    ...
    <tables-tit-more :tablesData="repayList">
      ...
      <div slot="datas" v-for="(r,index) in repayDatas" :key="index" v-show="dmainData" id="tables">
        <dd>
          <input type="checkbox" :checked="r.checked" :value="r.refundApply.id?r.refundApply.id:''" @click="oneCheckbox(r,index)" class="checkbox1">
          ...
          <div @click="goDetail(r.loan.id)" class="link-blue">查看</div>
        </dd>
      </div>
    </tables-tit-more>
    <!--统计-->
      <div class="bordm" slot="countData">
        <div class="repay-right bg-yellow">
          ...
          <span>融资总金额：￥{{countDatas.financ}}  元</span>
...
export default({
  ...
  methods: {
    getDatas1 () {
      // 已还款待平台确认接口
      this.$http.get('/api/repayment/repay/apply/list?page=1&&loanStaus=0')
        .then(res => {
