               ...
               <div class="goods1">
                 <div class="company-info">
                   <dl>
                     ...
                     <dd><input type="text" name="" v-model="name">
                      <div>
                         <span v-if="name==''">不能为空</span>
                       </div>
                     </dd>
                   </dl>
                   <dl>
         ...
         <div class="piaoju" style="position: relative;">
          <dl>
           ...
           <dd>
            ...
            <!-- <div class="btn-file">
              <div class="btn-zi btn-zi-big float-right">点击上传付款水单</div>
          ...
          <dl>
           ...
           <dd>
            <div class="pic-shouhui">
              <a :href="imgsSrc2"><img :src="imgsSrc2 ||  '/static/img/def_7.png'" v-if="showImg1"></a>
...
export default({
  ...
  methods: {
    ...
    addGoods () {
      let _this = this
      // 调添加商品的接口
      this.$http.post('/api/product/add', {'name': _this.name, 'hsNumber': _this.hsNumber, 'price': _this.price, 'rate': _this.rate, 'quantity': _this.quantity, 'totalPrice': _this.totalPrice})
        .then(res => {
    ...
    goApp2 () {
      ...
      // } else {
        ...
        // 条件判断
      if (_this.contract === '') {
        _this.$alert('合同号不能为空', '提示信息', {
          confirmButtonText: '确定'
        })
      }
      if (_this.customsDecl === '') {
        _this.$alert('报关单号', '提示信息', {
          confirmButtonText: '确定'
        })
      }
      if (_this.passState === '') {
        _this.$alert('海关放行状态', '提示信息', {
          confirmButtonText: '确定'
        })
      }
      if (this.declareTime === '') {
        _this.$alert('提示信息', '出口申报日期不能为空', {
          confirmButtonText: '确定'
        })
      }
      if (this.totalFin === '') {
        _this.$alert('提示信息', '出口企业不能为空', {
          confirmButtonText: '确定'
        })
      }
      if (this.declareSum === '') {
        _this.$alert('提示信息', '报关单金额不能为空', {
          confirmButtonText: '确定'
        })
      }
      if (this.factoryIdy === '') {
        _this.$alert('提示信息', '出口企业id不能为空', {
          confirmButtonText: '确定'
        })
      }
      if (this.settlement === '') {
        _this.$alert('提示信息', '退税等级不能为空', {
          confirmButtonText: '确定'
        })
      }
      if (this.earnings === '') {
        _this.$alert('提示信息', '收汇余额不能为空', {
          confirmButtonText: '确定'
        })
      }
      if (this.invoiceCode === '') {
        _this.$alert('提示信息', '发票代码不能为空', {
          confirmButtonText: '确定'
        })
      }
      if (this.customsDeclImg === '') {
        _this.$alert('提示信息', '报关单不能为空', {
          confirmButtonText: '确定'
        })
      }
      if (this.earningsImg === '') {
        _this.$alert('提示信息', '收汇凭证不能为空', {
          confirmButtonText: '确定'
        })
      }
      if (this.transferImg === '') {
        _this.$alert('提示信息', '结汇凭证不能为空', {
          confirmButtonText: '确定'
        })
      }
      if (this.receiptImg === '') {
        _this.$alert('提示信息', '发票凭证不能为空', {
          confirmButtonText: '确定'
        })
      }
      // 条件end
