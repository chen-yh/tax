...
export default({
  ...
  methods: {
    getDatas () {
      let _this = this
      this.$http.get('/api/factory/select/bycondition?factoryId=' + this.$route.query.id)
        .then(res => {
          console.log (res)
       ...
       _this.engagedTime = _this.getDateFormate(res.data.factoryEquipment[0].engagedTime)

       _this.capacity = res.data.factoryEquipment[0].capacity
       _this.foundTime = _this.getDateFormate(res.data.factoryEquipment[0].createTime)
       ...
       // _this.getDateFormate(res.data.factoryEquipment[0].engagedTime)
    ...
    getDateFormate (date2) {
      var oDate1 = new Date(date2);
      let _this = this
      let y1 = oDate1.getFullYear()
      // 获取完整的年份4位, 1970 - ????
      let m1 = oDate1.getMonth() + 1
      if (m1 < 10) {
        m1 = '0' + m1
      }
      // 获取当前月份( 0 - 11, 0代表1月 )
      // 所以获取当前月份是myDate.getMonth () + 1
      let d1 = oDate1.getDate()
      if (d1 < 10) {
        d1 = '0' + d1
      }
      // 获取当前日1 31
